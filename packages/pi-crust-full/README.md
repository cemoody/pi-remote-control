...
`pi-crust-full` is a meta-package. It contains no runtime code of its own — its `bin` simply re-execs the `pi-crust` CLI from the `pi-crust` package it pulled in. The four official extensions live in their own packages and are listed as `dependencies` of this one, so a single `npx pi-crust-full` invocation gets you the complete experience:
